...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * */
			// 获取最开始父类 new 出来的 beanFactory，告诉子类刷新内部 bean 工厂
			...
			try {
				// TODO: 2020/3/27 这一行并没有进行任何实现，我先把它注释掉
				...
				// 注册 spring 内置的 beanPostProcess 后置处理器
				...
				/*
				 * 非常重要
				 * bean实例化过程、ioc、注解支持、BeanPostProcessor的执行、Aop的入口
				 * 实例化所有剩余的（非延迟初始化）单例
				 * */
	 ...
	 * 往beanFactory中添加一些后置管理器，一些要忽略注入的接口啥的
	 * @param beanFactory the BeanFactory to configure
	 */
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// Tell the internal bean factory to use the context's class loader etc.
		beanFactory.setBeanClassLoader(getClassLoader());
		//bean表达式解释器，后面说  为了能够让我们的beanFactory去解析bean表达式
	 ...
	 * 实例化并调用所有注册的BeanFactoryPostProcessor的Bean，必须在单例实例化之前调用
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		/* 1.getBeanFactoryPostProcessors()获取自定义的：List<BeanFactoryPostProcessor> beanFactoryPostProcessors = new ArrayList<>();
		 * 如果你不在前面开始的时候调用 AnnotationConfigApplicationContext.addBeanFactoryPostProcesor()加进去是获取不到的，加了@Component也没用
		 * 2.invokeBeanFactoryPostProcessors:
		 * 调用自定义的 BDRegistryPostProcessor
		 * 调用spring内部的 BDRegistryPostProcessor
		 * 调用(整合排序过：自定义和spring内部实现) BDRegistryPostProcessor-> BeanFactoryPostProcessor
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 尽早初始化LoadTimeWeaverAware Bean实例对象,以便尽早注册其转换器
	...
	public Object getBean(String name) throws BeansException {
		// 检查 beanFactory目前是否处于活动状态，如果是 close 状态，那么就会抛出异常
