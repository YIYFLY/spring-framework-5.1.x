...
public class AnnotatedBeanDefinitionReader {
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry, Environment environment) {
		...
		// 创建条件计算器,这里创建了 beanFactory、environment（程序运行的环境对象）、resourceLoader（资源加载器）、classLoader（资源加载器的类加载器）
	 ...
	 * Register a bean from the given bean class, deriving its metadata from
	 * class-declared annotations.
	 * 读取bean的定义：这个类的作用把 BeanDefinition 和 beanName 放入到 map中
	 * @param beanClass the class of the bean
	 * @param instanceSupplier a callback for creating an instance of the bean
	 * (may be {@code null})
	 * @param name an explicit name for the bean
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		/**
		 * 这个类是通过注解过来的，所以bd类型也是注解类型-->AnnotatedGenericBeanDefinition
		 */
		 ...
		 * AnnotatonConfigApplicationContext在初始化的時候通调用用父类的构造方法
