...
public abstract class AnnotationConfigUtils {
	...
	public static void registerAnnotationConfigProcessors(BeanDefinitionRegistry registry) {
		// 注册注解的解析器,就是相当于读取 bean 的一些初始化定义
	...
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors(BeanDefinitionRegistry registry, @Nullable Object source) {
		// 获取了 DefaultListableBeanFactory，也就是beanFactory
		DefaultListableBeanFactory beanFactory = unwrapDefaultListableBeanFactory(registry);
		// 下面的两个判断是用来设置 beanFactory 的排序比较器和自动装配解析器的
		if (beanFactory != null) {
		...
		// 添加 5 个spring内部的bean
		// BeanDefinitio 的注册，这里很重要，需要理解注册每个bean的类型
		if (!registry.containsBeanDefinition(CONFIGURATION_ANNOTATION_PROCESSOR_BEAN_NAME)) {
			// 需要注意的是 ConfigurationClassPostProcessor 的类型是 BeanDefinitionRegistryPostProcessor 他实现了 BeanFactoryPostProcessor 这个接口
	...
	private static BeanDefinitionHolder registerPostProcessor(BeanDefinitionRegistry registry, RootBeanDefinition definition, String beanName) {
		// 设置权限，这个bean只供spring内部使用
		definition.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
		// 注册BeanDefinition,这里面包括了回去一级缓存中检查是不是已经存在
	...
	/**
	 * 检查常用的注解：Lazy、Primary、DependsOn、Role、Description
	 * 如果这个类上加了这些注解就设置值
